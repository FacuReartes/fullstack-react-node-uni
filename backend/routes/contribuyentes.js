  ...
  if (bajaFisica) {
    ...
    try {
      let data = await db.sequelize.query(
        "UPDATE contribuyentes SET Activo = case when Activo = 1 then 0 else 1 end WHERE Nro_Contribuyente = :Nro_Contribuyente",
        {
          replacements: { Nro_Contribuyente: +req.params.id },
        }
      );
      res.sendStatus(200);
    } catch (err) {
